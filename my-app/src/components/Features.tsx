...
// 1️⃣ Define allowed color keys
type ThemeKey = 'sage-green' | 'earth-tan' | 'dusty-rose';

// 2️⃣ Type the colorMap using Record
const colorMap: Record<
  ThemeKey,
  { bg: string; hoverBg: string; text: string; iconHover: string }
> = {
  'sage-green': {
    bg: 'bg-green-500',
...
// 3️⃣ Features typed with ThemeKey
interface Feature {
...
export default function App() {
  return (
    <div className="font-sans antialiased text-stone-800 bg-[#f8f5e4] min-h-screen rounded-xl shadow-vintage">
      <section id="features" className="py-20 bg-[#f8f5e4] rounded-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => {
              ...
              const colors = colorMap[feature.color]; // Now type-safe
              ...
              return (
                ...
                >
                  <div className="relative bg-white rounded-3xl p-8 h-full border border-stone-200 transition-colors duration-300">
                    ...
                    >
                      <Icon
                        className={`h-7 w-7 ${colors.text} transition-colors duration-300 ${colors.iconHover}`}
                      />
                    </div>
